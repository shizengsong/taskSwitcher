...
if (界面语言=="zh"){
	软件说明:=	"软件说明:`n`n1
			.改进windows任务窗口操作，鼠标移动到屏幕右下角，即可启动 `n`n2
			.不支持win10以上系统`n    也可以按键盘右侧ctrl键启动。 `n`n3
			.启动启动后，鼠标左键选择窗口，滚轮可滚动窗口`n    鼠标中键或右键可关闭窗口`n`n4
	...
	软件说明:=	"Apllication Direction:`n`n1
			...
			. Move the mouse to the bottom right corner of you screen
			, then you can switch the windows`n`n2
			...
			, scroll the windows by wheelup or wheeldown,and close a window by right click on it.`n`n4
